...
def update_vendor(update_data: VendorUpdate, db: Session = Depends(database.get_db)):
    ...
    # Actualizar la ubicación física si se proporciona
    if update_data.map_address:
        vendor.map_address = update_data.map_address
        
        # Auto-generar el iframe dinámico de Google Maps usando urllib.parse
        import urllib.parse
        address_query = f"{update_data.map_address}, Tampico, Tamaulipas"
        vendor.map_iframe_url = f"https://maps.google.com/maps?q={urllib.parse.quote(address_query)}&t=&z=16&ie=UTF8&iwloc=&output=embed"
